...
function RouteComponent() {
  ...
  const recentResponses = [
    { id: 1, name: "Alice Freeman", score: 10, comment: "Love the new WhatsApp integration! So fast.", date: "2m ago", status: "Promoter" },
    { id: 2, name: "Bob Smith", score: 8, comment: "It's good, but I wish it had more templates.", date: "15m ago", status: "Passive" },
    { id: 3, name: "Charlie Davis", score: 9, comment: "Very smooth experience.", date: "1h ago", status: "Promoter" },
    { id: 4, name: "Diana Prince", score: 4, comment: "I didn't receive the first message.", date: "3h ago", status: "Detractor" },
    { id: 5, name: "Evan Wright", score: 10, comment: "Perfect for our needs.", date: "5h ago", status: "Promoter" },
  ];

  const getScoreColor = (score: number) => {
    if (score >= 9) return "bg-green-500/15 text-green-600 dark:text-green-400 hover:bg-green-500/25 border-green-500/20";
    if (score >= 7) return "bg-yellow-500/15 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-500/25 border-yellow-500/20";
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border/50 pb-6">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground">
              Welcome back, <span className="font-semibold text-foreground">{session.data?.user.name}</span>. Here's your campaign overview.
            </p>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-7">
                ...
                <TableBody>
                  {recentResponses.map((response) => (
                    <TableRow key={response.id} className="group">
                      <TableCell>
                        <Badge variant="outline" className={`font-mono font-bold ${getScoreColor(response.score)}`}>
                          {response.score}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          <DropdownMenuTrigger className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8")}>
          ...
          <Card className="col-span-1 md:col-span-3 lg:col-span-2 border-border/50 shadow-sm flex flex-col h-full">
            ...
            <CardContent className="space-y-4 flex-1">
              ...
              <Button variant="ghost" className="w-full justify-start h-10 border border-transparent hover:border-border">
                   ...
                   <div className="flex items-start gap-3">
                     ...
                     <div>
                       ...
                       <p className="text-xs text-muted-foreground mt-1">
                         Your response rate is high! Consider adding a follow-up question for Promoters.
                       </p>
                     </div>
                   </div>
                 </CardContent>
