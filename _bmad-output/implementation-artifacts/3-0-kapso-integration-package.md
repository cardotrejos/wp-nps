...
- [ ] Task 1: Install Kapso Official SDK (AC: #4)
  - [ ] 1.1 Add `@kapso/whatsapp-cloud-api` to `packages/kapso/package.json`
  - [ ] 1.2 Run `bun install` to install the dependency
  - [ ] 1.3 Verify SDK is accessible and types are available

- [ ] Task 2: Create Real KapsoClient Implementation (AC: #1, #4, #5)
  - [ ] 2.1 Create `packages/kapso/src/client.ts` with `KapsoClient` class
  - [ ] 2.2 Implement constructor accepting `apiKey` and optional `baseUrl`
  - [ ] 2.3 Initialize internal `WhatsAppClient` from Kapso SDK
  - [ ] 2.4 Implement `sendSurvey()` - calls `client.messages.send()` with text message
  - [ ] 2.5 Implement `sendTestMessage()` - sends verification message
  - [ ] 2.6 Implement `getDeliveryStatus()` - queries message status
  - [ ] 2.7 Implement `createSetupLink()` - calls Platform API for onboarding
  - [ ] 2.8 Implement `getSetupLinkStatus()` - checks setup link status
  - [ ] 2.9 Implement `verifyWebhook()` using Kapso SDK's `verifySignature()`

- [ ] Task 3: Add Environment Configuration (AC: #4)
  - [ ] 3.1 Add `KAPSO_API_KEY` to `packages/env/src/server.ts` schema
  - [ ] 3.2 Add `KAPSO_WEBHOOK_SECRET` for signature verification
  - [ ] 3.3 Add `KAPSO_BASE_URL` with default `https://api.kapso.ai`
  - [ ] 3.4 Update `.env.example` files with new variables
  - [ ] 3.5 Add variables to test env setup

- [ ] Task 4: Create Kapso Client Factory (AC: #1, #2)
  - [ ] 4.1 Create `packages/kapso/src/factory.ts` with `createKapsoClient()`
  - [ ] 4.2 Factory returns `KapsoMockClient` when `NODE_ENV=test`
  - [ ] 4.3 Factory returns real `KapsoClient` in production
  - [ ] 4.4 Export factory from package index

- [ ] Task 5: Enhance Mock Client Webhook Verification (AC: #3)
  - [ ] 5.1 Add `mockValidSignature(signature: string)` method
  - [ ] 5.2 Add `mockInvalidSignature(signature: string)` method
  - [ ] 5.3 Update `verifyWebhook()` to check configured signatures
  - [ ] 5.4 Add `clearSignatureMocks()` method

- [ ] Task 6: Update Package Exports (AC: #1, #2)
  - [ ] 6.1 Export `KapsoClient` from `packages/kapso/src/index.ts`
  - [ ] 6.2 Export `createKapsoClient` factory function
  - [ ] 6.3 Keep separate mock export for test imports
  - [ ] 6.4 Update package.json exports if needed

- [ ] Task 7: Write Integration Tests (AC: #2, #3, #6)
  - [ ] 7.1 Create `packages/kapso/src/client.test.ts` for real client tests
  - [ ] 7.2 Test `KapsoClient` constructor with valid config
  - [ ] 7.3 Test `verifyWebhook()` with valid/invalid signatures using mock
  - [ ] 7.4 Test factory returns correct client based on environment
  - [ ] 7.5 Test mock client signature mocking methods
  - [ ] 7.6 Run all tests and verify passing

- [ ] Task 8: Update Existing Usages (AC: #1)
  - [ ] 8.1 Update `apps/server/src/lib/kapso.ts` to use factory
  - [ ] 8.2 Verify onboarding flow uses new client structure
  - [ ] 8.3 Verify test message functionality still works
...
- [ ] Real `KapsoClient` implements all `IKapsoClient` methods
- [ ] Factory returns mock in tests, real in production
- [ ] Webhook verification uses HMAC-SHA256
- [ ] Environment variables documented and validated
- [ ] All existing tests pass
- [ ] New integration tests for webhook verification
- [ ] No real API calls made in CI
